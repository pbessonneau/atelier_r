...
\item Charger les données correspondantes au format JSON
<<import>>=
reserve <- fromJSON("data/reserve/2014_reserve_parlementaire.json")
@


\item Faire un tableau les contributions par groupe, avec minimum, maximum ,moyenne et la médiane
...
\item Faire un graphique en boites à moustache des versements par groupe
