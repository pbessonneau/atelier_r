...
% Titre
\title{Introduction à R\\Délai médecins}
...
\section{Fichier de santé des Etats-Unis}

\begin{enumerate}
  \item Préparer la \emph{data.frame} pour la classification 
  \item Faire une classification après une ACP avec le fichier \emph{USHealth.csv} avec les
